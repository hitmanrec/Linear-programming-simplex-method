...
public class simplex_legacy
{
    ...
    public void gen_plane()
    {
        int i, j, it_num = 0;
        double A, B;
        while (!plane_is_valid() && function_is_undefined() && it_num < 15)
        {
            A = bv[i_lrow][1];
            B = istr[i_lcol];
            func -= A * B / alm;
        ...
        if (!function_is_undefined())
        {
            ...
            solText.text = "Целевая фукнция не ограничена, данная задача решений не имеет";
        }
            
        else if(it_num >= 15)
        {
            Debug.Log("Не удалось найти решение");
            solText.text = "Достигнут лимит итераций, решение найти не удалось";
        }
        else
        {
            Debug.Log("f(x) = " + func + " ");
